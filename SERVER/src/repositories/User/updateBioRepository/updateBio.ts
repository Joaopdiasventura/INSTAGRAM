import { IUpdateBioRepository, UpdateBioParams } from "../../../controllers/User/updateBioController/protocols";
...
export class UpdateBioRepository implements  IUpdateBioRepository {
    async update(params: UpdateBioParams): Promise<User | Message> {
        ...
        if (!user) {
            return {
                message: "Usuaário não encontrado"
            };
        }
        ...
        user.bio = (await prisma.user.findFirst({where: {email: params.email}})).bio;
        return user;
    }
