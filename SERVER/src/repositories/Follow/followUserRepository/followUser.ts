import { FollowUserParams, IFollowUserRepository } from "../../../controllers/Follow/followUserController/protocols";
...
export class FollowUserRepository implements IFollowUserRepository{
    async follow(params: FollowUserParams): Promise<Follow  | Message> {
        try {
            const user = await prisma.user.findFirst({where:{email: params.fk_user_email}});
            if(!user) return {message: 'Usuário não encontrado'};
            const user_ = await prisma.user.findFirst({where:{email: params.fk_user_email_}});
            if(!user_) return {message: 'Usuário que vai ser seguido não encontrado'};
